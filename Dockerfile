...
RUN groupadd -r cookbook && useradd -r -g cookbook cookbook
...
CMD ["uvicorn", "cookbook.main:app", "--host", "0.0.0.0", "--port", "8000", "--reload"]

# --- Stage 4: Production ---
FROM base AS production

# Copy built frontend assets to cookbook/static
COPY --from=frontend-builder /build/dist ./cookbook/static
...
RUN mkdir -p /app/uploads && chown cookbook:cookbook /app/uploads
